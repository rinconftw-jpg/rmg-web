    ...
    <>
      ...
      <section className="relative pt-40 pb-20 px-6 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto">
          <p className="text-[#F4E7C9] text-xs font-bold tracking-[0.3em] uppercase mb-3">What We Offer</p>
          ...
          {services.map((s, i) => (
            <div key={s.title} className={`grid md:grid-cols-2 gap-12 items-center py-16 ${i !== services.length - 1 ? 'border-b border-white/5' : ''}`}>
              ...
              <div className={`bg-[#141414] border border-white/5 rounded-xl p-8 ${i % 2 === 1 ? 'md:order-1' : ''}`}>
                <h3 className="text-xs font-bold text-[#F4E7C9] uppercase tracking-widest mb-5">Includes</h3>
                <ul className="space-y-3">
                  {s.items.map((item) => (
                    <li key={item} className="flex items-center gap-3 text-gray-300 text-sm">
                      <span className="w-1.5 h-1.5 bg-[#F4E7C9] rounded-full flex-shrink-0" />
                      {item}
                    </li>
                  ))}
                </ul>
