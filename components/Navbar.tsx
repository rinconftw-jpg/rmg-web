    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center">
          <span className="font-black text-lg tracking-wide">
            RMG <span className="text-[#F4E7C9]">CONSTRUCTION</span>
          </span>
        </Link>
        ...
        <div className="hidden md:flex items-center gap-3">
          <a href="tel:+15125551234" className="text-sm font-semibold text-[#F4E7C9] hover:text-[#F4E7C9] transition-colors">
            CALL NOW
          </a>
      ...
      {menuOpen && (
        <div className="md:hidden navbar-glass mt-2 px-6 py-4 flex flex-col gap-4">
          {links.map((l) => (
            <Link key={l.href} href={l.href} onClick={() => setMenuOpen(false)} className="text-gray-300 hover:text-[#F4E7C9] font-medium transition-colors">
              {l.label}
            </Link>
          ))}
          <div className="flex flex-col gap-2 pt-2 border-t border-white/10">
            <a href="tel:+15125551234" className="text-[#F4E7C9] font-bold">CALL NOW</a>
